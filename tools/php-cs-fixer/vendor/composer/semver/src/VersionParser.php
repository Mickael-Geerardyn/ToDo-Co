...
class VersionParser
{
    ...
    public function normalize($version, $fullVersion = null)
    {
        ...
        if (preg_match('{^v?(\d{1,5})(\.\d++)?(\.\d++)?(\.\d++)?' . self::$modifierRegex . '$}i', $version, $matches)) {
            $version = $matches[1]
                ...
                . (empty($matches[3]) ? '.0' : $matches[3])
                . (empty($matches[4]) ? '.0' : $matches[4]);
            $index = 5;
        ...
        if (isset($index)) {
            ...
            if (isset($matches[$index + 2]) && $matches[$index + 2] !== '') {
                $version .= '-dev';
            }
    ...
    private function parseConstraint($constraint)
    {
        ...
        if (preg_match('{^v?(\d++)(?:\.(\d++))?(?:\.(\d++))?(?:\.[xX*])++$}', $constraint, $matches)) {
            ...
            return [new Constraint('>=', $lowVersion), new Constraint('<', $highVersion)];
        }
        ...
        if (preg_match('{^(?P<from>' . $versionRegex . ') +- +(?P<to>' . $versionRegex . ')($)}i', $constraint, $matches)) {
            ...
            $empty = fn($x) => ($x === 0 || $x === '0') ? false : empty($x);

            if ((!$empty($matches[12]) && !$empty($matches[13])) || isset($matches[15]) && $matches[15] !== '' || isset($matches[17]) && $matches[17] !== '' || isset($matches[18]) && $matches[18] !== '') {
        ...
        if (preg_match('{^(<>|!=|>=?|<=?|==?)?\s*(.*)}', $constraint, $matches)) {
            try {
                try {
                    ...
                    if (str_ends_with($matches[2], '-dev') && preg_match('{^[0-9a-zA-Z-./]+$}', $matches[2])) {
                ...
                if ($op !== '==' && $op !== '=' && !empty($stabilityModifier) && self::parseStability($version) === 'stable') {
                    ...
                    if (!preg_match('/-' . self::$modifierRegex . '$/', strtolower($matches[2])) && !str_starts_with($matches[2], 'dev-')) {
                        $version .= '-dev';
                    }
                }
